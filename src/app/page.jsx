...
export default function Home() {
  ...
  const fileData =
    ...
    data
      ...
      .filter((data) =>
        data.name.split(".")[0].toLowerCase().includes(search.toLowerCase())
      );
    ...
    <>
      <section className="w-[90%] mx-auto">
        <header className="flex justify-between items-center mt-24">
          <div
            className="flex items-center border rounded px-3 py-2 gap-10 text-gray-500 cursor-pointer relative"
        ...
        {sort && (
          <div className="border mt-2 rounded-md p-4 max-w-max">
            <p className="cursor-pointer" onClick={handleFilterByName}>
              By name
            </p>
            <hr className="my-1" />
            <p className="cursor-pointer" onClick={handleFilterByDate}>
              By time created{" "}
            </p>
          </div>
        )}
